...
function pill(status: TicketStatus) {
  ...
  if (status === "CHECKOUT") return { bg: "rgba(255,255,255,0.08)", bd: "rgba(255,255,255,0.18)", fg: "rgba(255,255,255,0.92)", label: "CHECKOUT" };
...
export default function TablesPage() {
  ...
  // ✅ Oculta la mesa extra (MESA 8) por ahora sin tocar BD
  const visibleTables = useMemo(() => {
    return (tables || []).filter((t) => t.name !== "MESA 8");
  }, [tables]);
    ...
    <div style={S.page}>
      ...
      <div style={S.header}>
        ...
        <div style={S.actions}>
          {isAdmin && (
            <>
              <button style={btnGhost()} onClick={() => router.push("/products")}>
                Productos
              </button>
              <button style={btnGhost()} onClick={() => router.push("/users")}>
                Usuarios
              </button>
              <button style={btnGhost()} onClick={() => router.push("/reports")}>
                Reportes
              </button>
            </>
          )}
              ...
              <div style={S.body}>
                {occupied ? (
                  <>
                    ...
                    <div style={{ ...S.pill, background: p!.bg, borderColor: p!.bd, color: p!.fg }}>{p!.label}</div>
                  ...
                  <>
                    ...
                    <button style={{ ...btnPrimary(), width: "100%", marginTop: 10 }} onClick={() => openTicket(t.id)} disabled={loading}>
                      Iniciar
                    </button>
                  </>
                )}
...
const S: Record<string, React.CSSProperties> = {
  ...
  brandTitle: { fontSize: 20, fontWeight: 600, letterSpacing: 0.2, color: "rgba(255,255,255,0.96)" },
  ...
  tableName: { fontSize: 18, fontWeight: 600, letterSpacing: 0.2, color: "rgba(255,255,255,0.96)" },
  typeTag: {
    fontSize: 12,
    padding: "6px 10px",
    borderRadius: 999,
  ...
  dim: { color: "rgba(255,255,255,0.60)", fontSize: 13, fontWeight: 400 },
  kind: { color: "rgba(255,255,255,0.86)", fontSize: 13, fontWeight: 500, letterSpacing: 0.2 },
  free: { fontSize: 14, color: "rgba(255,255,255,0.70)", fontWeight: 400 },
